...
(function () {
  ...
  // ---- helpers (카드 마크업을 메인/리스트와 동일 스타일로) ----
  const formatPrice = (v) =>
  ...
  const CONDITION_LABEL = {
    NEW: '새상품', LIKE_NEW: '사용감 거의 없음', GOOD: '상태 좋음', FAIR: '보통', POOR: '사용감 많음'
  };
  const TRADE_LABEL = { MEETUP: '직거래', DELIVERY: '택배', BOTH: '직거래/택배' };
  const mapLabel = (v, map) => {
    if (v == null) return '';
  ...
  const renderList = (list) => {
    ...
    grid.innerHTML = list.map((p) => {
      const status = String(p.status || 'ACTIVE').toUpperCase(); // ACTIVE | RESERVED | SOLD
      const img = Array.isArray(p.photoUrls) && p.photoUrls.length
        ? toImgUrl(p.photoUrls[0])
        : (p.photos && p.photos[0]?.url ? toImgUrl(p.photos[0].url) : toImgUrl(null));

      const title = (p.title ?? '').trim();
      const price = formatPrice(p.price);
      const cond  = mapLabel(p.itemCondition, CONDITION_LABEL);
      const trade = mapLabel(p.tradeType, TRADE_LABEL);
      const negoY = String(p.negotiable || '').toUpperCase() === 'Y';
      const regionName = p.regionName || p.region?.name || p.addressText || '';

      return `
        <a class="product-card" href="/productDetail.html?id=${encodeURIComponent(p.listingId)}" title="${title}">
          ${status === 'RESERVED' || status === 'SOLD'
            ? `<div class="ribbon${status === 'SOLD' ? ' sold' : ''}">${status === 'SOLD' ? '판매완료' : '예약중'}</div>`
            : ''}
          ...
          <div class="product-info">
            ...
            <div class="meta-row">
              ${cond        ? `<span class="chip cond-new">${cond}</span>` : ''}
              ${trade       ? `<span class="chip trade">${trade}</span>`   : ''}
              ${negoY       ? `<span class="chip neg-y">네고가능</span>`     : ''}
              ${regionName  ? `<span class="chip">${regionName}</span>`     : ''}
            </div>
          </div>
        </a>
      `;
